...
class temp(wrapper_seq_base):
    ...
    async def body(self):
        ...
        await self.send_req(is_write=True, reg="RELOAD", data_condition=lambda data: data == 0x50)
        await self.send_req(is_write=True, reg="CFG", data_condition=lambda data: data == 0b010) # has to be periodic
        ...
        await self.send_req(is_write=True, reg="CTRL", data_condition=lambda data: data & 0b11111 == 0b1110)
        await self.send_req(is_write=True, reg="CTRL", data_condition=lambda data: data & 0b11111 == 0b1100)
